...
const BrowseTalks = () => {
  const [talks, setTalks] = useState([
    // Example talks data (same as provided in the original code)
    {
      "id": "12",
  ...
  const [interestedTalks, setInterestedTalks] = useState(() => {
    const savedInterestedTalks = localStorage.getItem('interestedTalks');
    return savedInterestedTalks ? JSON.parse(savedInterestedTalks) : [];
  });
  ...
  }, [location.search, selectedSession, talks]);
  ...
  const toggleInterest = (talkId) => {
    let updatedInterestedTalks;
    if (interestedTalks.includes(talkId)) {
      updatedInterestedTalks = interestedTalks.filter(id => id !== talkId);
    } else {
      updatedInterestedTalks = [...interestedTalks, talkId];
    }

    setInterestedTalks(updatedInterestedTalks);
    localStorage.setItem('interestedTalks', JSON.stringify(updatedInterestedTalks));
  };

  const sessions = ['All', 'A', 'B', 'C'];
  ...
  const styles = {
    ...
    interestButtonHovered: {
      backgroundColor: '#0056b3',
    },
    interestButtonInterested: {
      backgroundColor: '#28a745', // Green color for interested talks
    },
    interestButtonInterestedHovered: {
    ...
    <div style={styles.mainContainer}>
      <div style={styles.content}>
        ...
        <div style={styles.searchBar}>
          <label style={styles.searchLabel}>Filter by session: </label>
          <select
          ...
          >
            {sessions.map((session) => (
              <option key={session} value={session}>
                {session === 'All' ? 'All Sessions' : `Session ${session}`}
              </option>
            ))}
          </select>
        </div>
        ...
        {filteredTalks.length === 0 ? (
          <p>No talks found for this search and/or session.</p>
        ) : (
          <ul>
            {filteredTalks.map((talk) => (
              <li key={talk.id} style={styles.talkItem}>
                <div style={styles.talkInfo}>
                  <strong>{talk.title}</strong> by {talk.speaker} (Session {talk.session})
                </div>
                <button
                  style={
                    interestedTalks.includes(talk.id)
                      ? { ...styles.interestButton, ...styles.interestButtonInterested }
                      : styles.interestButton
                  }
                  onClick={() => toggleInterest(talk.id)}
                  onMouseOver={(e) => e.target.style.backgroundColor = interestedTalks.includes(talk.id) ? styles.interestButtonInterestedHovered.backgroundColor : styles.interestButtonHovered.backgroundColor}
                  onMouseOut={(e) => e.target.style.backgroundColor = interestedTalks.includes(talk.id) ? styles.interestButtonInterested.backgroundColor : styles.interestButton.backgroundColor}
                >
                  {interestedTalks.includes(talk.id) ? 'Unmark as Interested' : 'Mark as Interested'}
                </button>
              </li>
            ))}
          </ul>
        ...
        <div>
          <h3>Your Interested Talks</h3>
          <ul>
            {talks.filter(talk => interestedTalks.includes(talk.id)).map((talk) => (
              <li key={talk.id}>
                <strong>{talk.title}</strong> by {talk.speaker} (Session {talk.session})
              </li>
            ))}
