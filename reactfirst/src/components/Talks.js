...
const talksData = [
  ...
  function Talks() {
    const [visibleTalks, setVisibleTalks] = useState(4); // Initially show 4 talks
  
    const loadMoreTalks = () => setVisibleTalks(prev => prev + 4); // Load more talks on click
    ...
    return (
      <section style={styles.section}>
        <h2 style={styles.header}>Speakers and Talks</h2>
        <div className="talks-list">
          {talksData.slice(0, visibleTalks).map((talk) => (
            <div style={styles.talkCard} key={talk.id}>
              <div style={styles.talkInfo}>
                <h3 style={styles.talkTitle}>{talk.title}</h3>
                <p><strong>_Id:</strong> {talk._id}</p>
                <p><strong>Id:</strong> {talk.id}</p>
                <p><strong>Speaker:</strong> {talk.speaker}</p>
                <p><strong>Time:</strong> {talk.time}</p>
                <p><strong>Session:</strong> {talk.session}</p>
                <p><strong>Tags:</strong> {talk.tags.length > 0 ? talk.tags.join(', ') : 'No tags available'}</p>
                <p>{talk.description}</p>
                <p><strong>Ratings:</strong> {talk.ratings.length > 0 ? talk.ratings.join(', ') : 'No ratings available'}</p>
              </div>
            </div>
          ))}
        </div>
        {visibleTalks < talksData.length && (
          <button
            style={styles.loadMoreButton}
            onClick={loadMoreTalks}
            onMouseOver={(e) => e.target.style.backgroundColor = styles.loadMoreButtonHover.backgroundColor}
            onMouseOut={(e) => e.target.style.backgroundColor = styles.loadMoreButton.backgroundColor}
          >
            Load More
          </button>
        )}
